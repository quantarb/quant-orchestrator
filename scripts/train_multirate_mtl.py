...
def main() -> None:
    ...
    parser.add_argument("--options-per-side", type=int, default=5, help="Maximum sampled calls and puts each per underlying/year for warehouse training and backtests (0 disables options; default: 5)")
    ...
    if args.min_market_cap is not None:
        from quant_orchestrator.research_tools.warehouse_multirate_training import run_warehouse_training
        try:
            run_warehouse_training(args)
        except Exception as exc:
            message=str(exc) if isinstance(exc,ValueError) else type(exc).__name__
            if getattr(args,'_warehouse_run_started',False):
                import traceback
                (args.output_dir/'failure.txt').write_text(traceback.format_exc())
                (args.output_dir/'status.json').write_text(json.dumps(dict(stage='failed',error=message),indent=2))
            print('error: '+json.dumps(message))
            print('help: '+json.dumps('Inspect the run status and coverage artifacts, repair the source, then launch with a new --output-dir'))
            raise SystemExit(1) from None
        return
    if not args.inference_only and not args.resume_training:
        parser.error('New training requires --min-market-cap and builds documents on demand; --corpus is only for checkpoint evaluation or exact resume')
    ...
    epoch_clocks = {}
    from quant_orchestrator.research_tools.multirate_training_step import make_training_step, issuer_stream_inputs
    training_step = make_training_step(
        args=args,
        trainer=trainer,
        device=device,
        annual_state=annual_state,
        reconstruction_widths=reconstruction_widths,
        feature_family_dimensions=feature_family_dimensions,
        sparse_input_families=sparse_input_families,
        raw_sparse_columns=raw_sparse_columns,
        family_names=family_names,
        asset_class_ids=asset_class_ids,
        enabled_document_tasks=enabled_document_tasks,
        prediction_names=prediction_names,
        expected_task_names=expected_task_names,
        mrl_dimensions=mrl_dimensions,
        task_observations=task_observations,
        task_family_observations=task_family_observations,
        task_loss_sums=task_loss_sums,
        epoch_clocks=epoch_clocks,
        alignment_loss=matryoshka_alignment_loss,
    )
