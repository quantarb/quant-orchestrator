...
def compare_epoch_oracle_gate(directory: Path, *, mode="strict"):
    if mode not in ("strict", "directional"):
        raise ValueError(f"Unknown Oracle gate mode: {mode}")
    artifact_name="oracle_gate" if mode=="strict" else "oracle_directional_gate"
    ...
    for period in sorted({r['period'] for r in baseline}):
        ...
        output=directory/artifact_name/period;output.parent.mkdir(exist_ok=True)
        if (output/'results.json').exists():
            ...
            gated=run_existing_multirate_backtest(scores,pl.scan_parquet(prices),output,oracle_gate=True,oracle_gate_mode=mode)
        for result in gated:
            before=next(r for r in original if r['side']==result['side'])
            comparisons.append(dict(oracle_gate_mode=mode,period=period,side=result['side'],
    ...
    temporary=directory/f'{artifact_name}_comparison.tmp'
    temporary.write_text(json.dumps(comparisons,indent=2));temporary.replace(directory/f'{artifact_name}_comparison.json')
    return comparisons
