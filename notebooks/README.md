...
- `mult-ml-frameworks/sample_model_training.ipynb` demonstrates CUDA-first toy model training across `yfinance` and `fmp` using Quant Warehouse adjusted OHLCV features and optimal-trading labels: RAPIDS cuML RandomForest, PyTorch autoencoder, and FlairNLP multitask text classification/regression with a tiny pretrained transformer.
