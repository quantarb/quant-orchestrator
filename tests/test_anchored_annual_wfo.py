...
class _Classifier:
    ...
    def predict_proba_frame(self, frame, features):
        ...
        return pd.DataFrame(
            {
                "prob__oracle_long": np.full(len(frame), 0.7),
                "prob__oracle_short": np.full(len(frame), 0.3),
            },
            index=frame.index,
        )
...
def test_annual_wfo_uses_prior_rows_bounds_oos_and_restarts(tmp_path):
    ...
    labels = pd.DataFrame(
        {
            "symbol": "AAPL",
            "date": dates[:3],
            "collapsed_label": ["oracle_long", "oracle_short", "oracle_long"],
        }
    )
    lineage = build_dataset_lineage_manifest(
        ...
        provider="test",
        available_at_cutoff="2022-01-03",
    ...
    config = AnchoredAnnualWFOConfig(
        output_dir=tmp_path / "wfo",
        run_id="unit",
        target_id="oracle-side",
        input_lineage_paths=(lineage_path,),
    ...
    assert fit_dates == [
        (pd.Timestamp("2019-01-02"), "2019-12-31"),
        (pd.Timestamp("2020-01-02"), "2020-12-31"),
    ]
    ...
    assert first.annual_metrics[["test_year", "model_id"]].values.tolist() == [[2020, "test.unit"]]
...
def test_annual_wfo_fits_feature_selection_on_prior_year_training_rows(tmp_path):
    dates = pd.to_datetime(["2018-01-02", "2019-01-02", "2020-01-02"])
    panel = pd.DataFrame(
        {
            "symbol": "AAPL",
            "date": dates,
            "available_in_train": [1.0, 2.0, 3.0],
            "oos_only": [np.nan, np.nan, 99.0],
        }
    )
    labels = pd.DataFrame(
        {
    ...
    lineage = build_dataset_lineage_manifest(
        ...
        dataset_kind="feature_panel",
        provider="test",
        available_at_cutoff="2020-01-03",
        recipe_id="features-v1",
        recipe={"family": "unit"},
    )
    lineage_path = write_dataset_lineage_manifest(lineage, tmp_path / "lineage.json")
    ...
    def classifier_factory(frame, features, config):
        del frame, config
        fitted_features.append(features)
        return _Classifier(_Encoder(np.asarray(["oracle_long", "oracle_short"])))

    run_anchored_annual_wfo(
        lambda: [FeatureFamilyBatch("test", "unit", panel, ("available_in_train", "oos_only"))],
        labels,
        config=AnchoredAnnualWFOConfig(
            output_dir=tmp_path / "wfo",
            run_id="unit",
