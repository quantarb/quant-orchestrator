...
Training-only event supervision can be materialized as a sorted snapshot by `StreamingSupervision.materialize`; `research_tools/supervision_index.py` caches exact-date label arrays with a 128 MiB limit and a bounded date-window fallback. This avoids repeated source aggregation without caching learned outputs or changing event cutoffs.
