...
Quant research is fragile because the same trading idea can behave differently depending on the data vendor, feature and label pipeline, and backtesting framework used to test it. Price adjustments, missing rows, corporate-action handling, trading calendars, order timing, fill simulation, fees, slippage, and framework-specific accounting can all change measured performance. `quant-orchestrator` should make those differences visible by making multi-data-vendor and multi-backtesting-framework research practical, without pretending there is one universal best vendor or one universal best engine for every strategy.
